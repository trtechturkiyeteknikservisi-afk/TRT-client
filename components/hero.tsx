  ...
  return (
    <>
      <section 
        ...
        className="relative xl:min-h-screen w-full overflow-hidden bg-background flex flex-col pt-8 xl:justify-center pb-10 xl:pb-16"
      >
        {/* Dynamic Backgrounds */}
        ...
        <div className="relative container mx-auto px-4 w-full flex flex-col xl:flex-row items-center xl:justify-between gap-8 xl:gap-16 z-10">
          ...
          {/* Contact Form Section (Desktop-only) */}
          <div className="hidden xl:block xl:w-[540px] relative z-20 space-y-6">
      ...
      {/* Mobile-only Contact Form & Trust Section (Outside the section/banner background) */}
      <div className="xl:hidden bg-background py-12 px-4 border-t border-border/20">
        <div className="container mx-auto max-w-xl space-y-6">
          <ContactForm isHeroMini={true} />
          
          {/* Mobile-only Trust Badges & Indicators */}
          <div className="space-y-6">
            ...
            <div className="flex flex-row items-center justify-center gap-8 py-4">
              <div className="flex items-center gap-4 text-foreground font-black text-xs tracking-widest uppercase">
                <span className="text-primary bg-primary/10 px-2 py-1 rounded-md">0{current + 1}</span>
                <div className="w-8 h-px bg-foreground/20" />
                <span className="bg-foreground/10 px-2 py-1 rounded-md">0{banners.length}</span>
              </div>
                ...
                {banners.map((_, index) => (
                  <button
                    ...
                    className={cn(
                      ...
                      current === index ? "bg-primary w-12" : "bg-foreground/20"
                    )}
                  />
                ))}
              </div>
