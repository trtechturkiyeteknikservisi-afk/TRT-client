...
export default async function Home({
  ...
  const stats = [
    {
      ...
      value: locale === 'tr' ? '15B+' : '15K+'
    },
    {
      label: t('stat_devices'),
      value: locale === 'tr' ? '20B+' : '20K+'
    },
    {
  ...
  try {
    ...
    console.warn(
      "Could not prefetch banners server-side:",
      message
    );
  }
  ...
  return (
    <main className="min-h-screen selection:bg-primary selection:text-primary-foreground overflow-x-hidden">
      ...
      <HomeSections
        stats={stats}
        trustBadgeLabel={t_trust('badge_label')}
      />
    </main>
  );
}
