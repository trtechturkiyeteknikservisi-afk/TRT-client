  ...
  return (
    <div className="space-y-12 pb-12">
        ...
        {statCards.map((card, index) => (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-8">
               <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-muted text-foreground ring-1 ring-border/50 group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-3 bg-card/50 backdrop-blur-xl p-10 rounded-md border-2 border-border/50 shadow-xl shadow-black/5 space-y-10 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Real-time engagement data (Page Views)</p>
                  </div>
                  <div className="flex bg-muted/50 p-1.5 rounded-lg border border-border/50">
                      <button className="px-5 py-2.5 bg-primary text-primary-foreground shadow-lg shadow-primary/20 rounded-md text-[10px] font-black uppercase tracking-widest">7 Days</button>
                      <button className="px-5 py-2.5 hover:bg-background rounded-md text-[10px] font-black uppercase tracking-widest text-muted-foreground transition-all">30 Days</button>
                  </div>
              </div>
              
              <div className="h-64 flex items-end justify-between px-4 pt-10 border-b-2 border-border/30">
                  {(stats?.traffic || [0,0,0,0,0,0,0]).map((item: any, i: number) => {
                      const count = typeof item === 'number' ? item : (item.count || 0);
                      const maxCount = Math.max(...(stats?.traffic?.map((t: any) => t.count) || [100]));
                      ...
                      return (
                        <div key={i} className="w-full mx-2 group relative flex flex-col justify-end">
                            <motion.div 
                              initial={{ height: 0 }}
                              animate={{ height: `${Math.max(h, 5)}%` }} // Minimum height of 5% for visibility
                            ...
                            <motion.div 
                              ...
                              className={cn(
                                "absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-md text-[10px] font-black shadow-lg border whitespace-nowrap z-10",
                                count > 0 
