...
export default async function PolicyDetailPage(props: {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const settingKey =
        slug === 'terms' ? 'policy' : slug;

      content =
        settings[`${settingKey}_${locale}`] ||
        settings[`${settingKey}_tr`] ||
        settings[`${settingKey}_en`] ||
        content;

      const path =
        settings[`${slug}_pdf_${locale}`] ||
        settings[`${slug}_pdf_tr`] ||
        settings[`${slug}_pdf_en`];
      ...
      if (path) {
        if (path.startsWith('http')) {
          ...
          if (
            ...
            const cleanUrl = API_URL.startsWith('.')
              ? API_URL.substring(1)
              : API_URL;
        ...
        <main className="pt-8 md:pt-12 pb-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-6xl mx-auto space-y-10">
            ...
            <div className="bg-card p-8 md:p-12 lg:p-16 rounded-xl border shadow-2xl shadow-black/5 prose prose-stone dark:prose-invert max-w-none">
              <div
                className="whitespace-pre-wrap font-bold leading-relaxed text-lg text-foreground/90 selection:bg-primary/10"
                dir={isRTL ? 'rtl' : 'ltr'}
              >
                {content}
              </div>
